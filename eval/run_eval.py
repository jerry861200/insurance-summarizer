...
from eval.metrics import CaseResult, evaluate_case, render_report
...
def main() -> int:
    ...
    if args.live:
        ...
        mode = f"live ({settings.anthropic_model})"
    elif args.mock:
        llm = _MockProvider()
    ...
    for case in cases:
        ...
        cr = evaluate_case(case["expected"], actual, case["case_name"])
    ...
    if args.output:
        ...
        output_path.write_text(f"<!-- Generated by `python -m eval.run_eval --{('live' if args.live else 'mock' if args.mock else 'no flag')}` -->\n\n")
